...
inspect half the keys in the list before finding the one we're looking for. The expected lookup time is therefore $cn/2$. Since $c/2$ is a 
...
before terminating. The time spent searching the list is therefore $cn$. Since $c$ is a constant, this can be expressed in $\Theta$ notation as $\Theta(n)$.

The expected lookup time in both the average and worst case is therefore $\Theta(n)$.
...
\begin{minipage}{\dimexpr\textwidth-2\fboxsep-2\fboxrule\relax}
\vspace{2mm}
3. Consider the problem of determining whether an arbitrary sequence $()$ of $n$ numbers contains repeated occurrences of some number. 
Show that this can be done in $x$ time, where $x$ stands for $x$.
\vspace{2mm}
\end{minipage}
}
}
